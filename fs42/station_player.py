...
from fs42.direct_streamer import DirectHLSStreamer
...
class StationPlayer:
    ...
    def __init__(self, station_config, input_check_fn, mpv=None):
        ...
        if not mpv:
            self._l.info("Starting direct FFmpeg HLS streamer instead of MPV")
            ...
            self.mpv = DirectHLSStreamer(hls_config)
    ...
    def show_web(self, web_config):
        """
        Streaming-version web channel handler.

        Original FS42 behavior:
            - start PySide web_render_runner
            - show the webpage as a local window
            - stop MPV

        Streaming behavior:
            - send the web_config to DirectHLSStreamer
            - DirectHLSStreamer renders it hidden with Xvfb + Chromium
            - Fire TV receives it as HLS
        """
        ...
        if not web_url:
            self._l.error(f"Web config has no web_url/url field: {web_config}")
            return PlayerOutcome(PlayerState.FAILED)

        # If the player backend supports streaming web capture, use it.
        # In our Fire TV setup, self.mpv is DirectHLSStreamer, not real MPV.
        if hasattr(self.mpv, "play_web"):
            self._l.info(f"Starting web channel through DirectHLSStreamer: {web_url}")

            self.current_playing_file_path = web_url
            ...
            try:
                self.mpv.play_web(web_config)
            except Exception as e:
                self._l.exception(e)
                self._l.error(f"Failed to start streaming web channel: {web_url}")
                return PlayerOutcome(PlayerState.FAILED)
            ...
            if duration:
                try:
                    stop_time = datetime.datetime.now() + datetime.timedelta(seconds=float(duration))
                    self._l.info(f"Streaming web content will auto-stop after {duration} seconds")
                except Exception:
                    stop_time = None
            ...
            while True:
                ...
                if stop_time and datetime.datetime.now() >= stop_time:
                    self._l.info("Streaming web content duration expired, stopping web capture")
                    try:
                        if hasattr(self.mpv, "stop_web"):
                            self.mpv.stop_web()
                        else:
                            self.mpv.stop()
                    except Exception as e:
                        self._l.warning(f"Error stopping streaming web channel: {e}")
                ...
                response = self.input_check_fn()
                if response:
                    self._l.info("Stopping streaming web channel due to player input/channel change")
                    try:
                        if hasattr(self.mpv, "stop_web"):
                            self.mpv.stop_web()
                        else:
        ...
        # Fallback for non-streaming/original FS42.
        # This should not run in your Fire TV streaming setup.
        self._l.warning("DirectHLSStreamer.play_web is not available; falling back to original PySide web renderer")

        if not WEB_RENDER_AVAILABLE:
            self._l.error("Web rendering not available - PySide6 not installed")
            msg = "Web rendering requires PySide6 to be installed and configured."
