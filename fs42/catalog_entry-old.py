...
class CatalogEntry:
    ...
    def __init__(self, path, duration, tag, hints=[], count=0, content_type="feature", media_type="video"):
        ...
        self.title = self._make_title_from_tag_folder(path, tag)
        self.count = count
        self.hints = hints
    ...
    @classmethod
    def _make_title_from_tag_folder(cls, path, tag):
        """
        Build a guide title like:

            Dragon Ball - Dragon Ball 057 DBOX CC 480p x264 SoM

        when the file is inside:

            .../am-shows/Dragon Ball/Dragon.Ball.057.DBOX.CC.480p.x264-SoM.mkv

        and the scheduled/catalog tag is:

            am-shows

        The "top folder" means the first directory underneath the tag folder.
        """
        file_title = cls._clean_title_part(path)

        try:
            parts = os.path.normpath(path).split(os.sep)
            tag_parts = os.path.normpath(str(tag)).split(os.sep)

            # Find the tag path inside the full file path.
            # Supports tags like:
            #   am-shows
            #   anime/am-shows
            tag_index = None

            for i in range(0, len(parts) - len(tag_parts) + 1):
                if parts[i : i + len(tag_parts)] == tag_parts:
                    tag_index = i
                    break
            ...
            top_folder_index = tag_index + len(tag_parts)

            # Need at least one folder between tag and file.
            if top_folder_index >= len(parts) - 1:
                return file_title

            show_folder = cls._clean_title_part(parts[top_folder_index])

            if not show_folder:
                return file_title

            # Avoid ugly duplicates like:
